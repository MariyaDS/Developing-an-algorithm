...
# When approved_users[0] and approved_devices[0] are displayed, they show the first elements of each list. Replacing 0 with another index displays the corresponding elements at that index in both lists.By changing the index, you get different elements from the lists at that specific position.
...
# Both the new user and the new device were added to the list of approved_users and approved_devices at the end of the list.
...
# The user "tshah" and their corresponding device "2ye3lzg" have been successfully removed from the approved_users and approved_devices lists.
# The elements of the two lists remain synchronized, with each username corresponding to the correct device ID at the same index.
...
# The message indicates that "sgilmore" is found in the approved_users list, and thus, the user is confirmed to have access to the system.
...
# This indicates that "sgilmore" is found at index 2 in the approved_users list. The index method correctly returns the position of "sgilmore" within the list.
...
# This output indicates that the device ID associated with the username "sgilmore" is "4n482ts". The code correctly finds the index of "sgilmore" in the approved_users list (which is 2) and uses this index to retrieve the corresponding device ID from the approved_devices list.
...
# The username "sgilmore" is found in the approved_users list.
# The device ID "4n482ts" matches the device ID assigned to "sgilmore" in the approved_devices list (at the same index).
# Therefore, the conditional statement evaluates to True, and both the approval message and the device assignment message are printed.
# This confirms that the username is approved and that the user has the correct device.
...
# This output indicates that the username "sgilmore" is found in the list of approved users (approved_users), and the corresponding device ID for "sgilmore" in the approved_devices list matches the device_id provided ("4n482ts"). Therefore, the user "sgilmore" is approved to access the system, and "4n482ts" is indeed their assigned device.
# 
# This matches the conditions specified in the if statement of the code.
...
# The username "sgilmore" is found in approved_users, but the provided device_id does not match the one stored in approved_devices for "sgilmore". Hence, it prints that the provided device is not assigned to the user.
# These outputs demonstrate how the function correctly validates the user and verifies the device assignment based on the input parameters.

# ## Conclusion
# 
# **What are your key takeaways from this lab?**

# Key takeaways from this lab include understanding how to manipulate lists in Python, particularly using indexing and conditional statements to verify user access based on usernames and device IDs. It emphasizes the importance of synchronization between lists and how to implement nested conditionals for structured decision-making in algorithms.
